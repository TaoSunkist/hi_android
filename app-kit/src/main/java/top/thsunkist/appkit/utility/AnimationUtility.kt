...
object AnimationUtility {
     ...
     * @param view 礼物小特效
     */
    fun giftBannerTransfer(view: View) {
        val current = 0.0
...
/*    fun fullscreenGiftEffectInSvga(effectSvga: SVGAImageView?, statusBarHeight: Int) {
        effectSvga?.let {
            val width: Int
            val height: Int
            var margin = 0
            val scaleWidth: Boolean
            val screenHeight = Dimens.screenHeight + statusBarHeight + Dimens.dpToPx(10)
            val scale = 0.562f //礼物宽高比
            val screenScale = Dimens.screenWidth.toFloat() / screenHeight.toFloat()
            if (screenScale > scale) {
                ...
                width = (screenHeight * scale + .5f).toInt()
                height = screenHeight
