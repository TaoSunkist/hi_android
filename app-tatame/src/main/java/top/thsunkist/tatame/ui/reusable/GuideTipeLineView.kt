...
class GuideTipeLineView : View {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (guideType == GuideType.CONFIRM) {
            canvas.drawLine(
                    startX,
                    startY * 2.5f,
                    stopX,
                    startY * 2.5f + Dimens.dpToPx(30).toFloat(),
                    paint)

            startX = stopX
            startY = startY * 2.5f + Dimens.dpToPx(30).toFloat()
            stopX = Dimens.dpToPx(45).toFloat() * 2
            stopY = startY
            ...
            canvas.drawCircle(
                    ...
                    circleRadius,
                    paint)
            canvas.drawText("确认", stopX + stopX * 0.12f, stopY + stopY * 0.1f, textPaint)
        }
    ...
    fun showGuideType(guideType: GuideType) {
        this.guideType = GuideType.CONFIRM
        invalidate()
    }
}