package top.thsunkist.brainhealthy.ui.reusable.viewcontroller.host
...
import com.zhimeng.tatame.ui.reusable.viewcontroller.util.animation.AnimationEndListener
...
open class ControllerHost(activity: BaseActivity) : FrameLayout(activity) {
    ...
    var controllers = ArrayList<ViewController>()
    ...
    fun push(
        ...
        var layoutParams = LayoutParams(viewController.view.layoutParams).apply {
            if (viewController.presentationStyle.fullscreen.not()) {
                ...
                /** viewController.processStatusBarCover() == 0
                表示viewcontroller自己处理高度 */
                if (viewController.processStatusBarCover() == 0) {
                    topMargin = viewController.processStatusBarCover()
                }
            }
        ...
        if (animated) {
            ...
            /* if needn't animation, then appered of view was shown, an transaction was over */
            if (viewController.presentationStyle.dim) {
                cover.alpha = dimRatio
            }
            controllers.add(viewController)
    ...
    private fun onlyAllowTopViewToReceiveTouch() {
        ...
        for (index in childCount - 1 downTo 0) {
            getChildAt(index).setOnTouchListener { _, _ -> true }
        }
